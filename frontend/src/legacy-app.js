...
async function bootstrap() {
  ...
  try {
    ...
    await runOptionalBootStep("加载流量监控", () => refreshFlowMonitor(true), { retries: 1, timeout: 16000 });
...
function buildRealtimeStatusText(data) {
  ...
  lines.push(`[流量监控] ${flowItems.length} 项`);
...
function renderFlowMonitorTable() {
  ...
  const rows = limited.map((item) => ({
    cells: [
      ...
      `<button class="btn sm" type="button" data-flow-detail="${escapeHTML(item.key)}">详情</button>`,
    ],
  }));
  renderTable("flowMonitorList", ["进程名称", "协议/连接", "状态", "最后活跃", "PID", "流量摘要", "操作"], rows, true);
...
async function refreshFlowMonitor(force = false) {
  ...
  try {
    const res = await fetchWithTimeout("/api/traffic?packet_limit=1&http_limit=1", 12000);
    let payload = {};
    try {
    ...
    const modeText = unsupported ? "连接态（无抓包）" : "抓包态";
    const items = connections.map((item) => {
      ...
      return {
        ...
        raw: item,
        detail: unsupported
          ? `${modeText} | 字节/包统计不可用（需启用 CGO + Npcap）`
          : `${modeText} | 入/出流量 ${bytes(bytesIn)} / ${bytes(bytesOut)} | 入/出包 ${num(packetsIn)} / ${num(packetsOut)}`,
      };
    });
...
function localizeFlowConnectionStatus(raw) {
  ...
  if (value === "ESTABLISHED") return "已建立";
  if (value === "LISTEN") return "监听中";
  if (value === "SYN_SENT" || value === "SYN_RECV") return "握手中";
  if (["FIN_WAIT1", "FIN_WAIT2", "CLOSE_WAIT", "CLOSING", "LAST_ACK", "TIME_WAIT"].includes(value)) return "关闭中";
  if (value === "CLOSED" || value === "CLOSE") return "已关闭";
  if (value === "UNKNOWN" || value === "NONE") return "未知";
  return value;
}
...
function buildFlowMonitorSummary(items) {
  ...
  return `TCP ${num(tcp)} | UDP ${num(udp)} | 已建立 ${num(established)} | 监听 ${num(listen)}`;
}
...
function openFlowMonitorDetailModalByKey(key) {
  ...
  if (!item) {
    showAppToast("未找到该连接详情，可能已过期", "warning");
    return;
  }
  openFlowMonitorExplorerModal(target, "流量连接详情（支持搜索过滤）", {
    seedItem: item,
  });
}
...
function buildFlowMonitorDetailRows(item) {
  return [
    ["进程名称", escapeHTML(item.name || "-")],
    ["PID", escapeHTML(item.finished_at || "-")],
    ["进程路径", escapeHTML(item.exePath || "-")],
    ["协议", escapeHTML(String(item.protocolValue || "-").toUpperCase())],
    ["状态", escapeHTML(item.statusText || item.statusRaw || "-")],
    ["本地地址", escapeHTML(item.localAddress || "-")],
    ["远端地址", escapeHTML(item.remoteAddress || "-")],
    ["最后活跃", escapeHTML(item.started_at || "-")],
    ["连接键", `<code>${escapeHTML(item.connectionKey || "-")}</code>`],
    ["入流量", escapeHTML(bytes(item.bytesIn || 0))],
    ["出流量", escapeHTML(bytes(item.bytesOut || 0))],
    ["入包数", escapeHTML(num(item.packetsIn || 0))],
    ["出包数", escapeHTML(num(item.packetsOut || 0))],
    ["采集模式", escapeHTML(item.captureMode || "-")],
    ["摘要", escapeHTML(item.detail || "-")],
  ];
}

function openFlowMonitorExplorerModal(initialKey = "", title = "流量连接详情（支持搜索过滤）", options = {}) {
  ...
  const html = `
    <section class="flow-monitor-explorer">
      <div class="row">
        <input id="flowMonitorDetailSearch" class="input" placeholder="搜索进程 / PID / 地址 / 协议 / 状态 / 路径" value="${escapeHTML(keyword)}" />
        <select id="flowMonitorDetailProtocol" class="input sm">
          <option value="all" ${protocol === "all" ? "selected" : ""}>全部协议</option>
        ...
        <select id="flowMonitorDetailStatus" class="input sm">
          ...
          <option value="established" ${status === "established" ? "selected" : ""}>已建立</option>
          <option value="listen" ${status === "listen" ? "selected" : ""}>监听中</option>
          <option value="closing" ${status === "closing" ? "selected" : ""}>关闭中</option>
          <option value="unknown" ${status === "unknown" ? "selected" : ""}>未知</option>
        </select>
        <select id="flowMonitorDetailSort" class="input sm">
          <option value="time_desc" ${sort === "time_desc" ? "selected" : ""}>最新优先</option>
          <option value="bytes_desc" ${sort === "bytes_desc" ? "selected" : ""}>流量优先</option>
          <option value="packets_desc" ${sort === "packets_desc" ? "selected" : ""}>包数优先</option>
          <option value="pid_asc" ${sort === "pid_asc" ? "selected" : ""}>PID 升序</option>
          <option value="name_asc" ${sort === "name_asc" ? "selected" : ""}>进程名 A-Z</option>
        </select>
  ...
  const renderDetailPanel = () => {
    ...
    if (!item) {
      panel.innerHTML = '<div class="hint">当前筛选结果为空，无法显示连接详情</div>';
      return;
    }
    const rows = buildFlowMonitorDetailRows(item);
    panel.innerHTML = `
      <div class="table flow-monitor-detail-kv">${renderTableHTML(["字段", "值"], rows, true)}</div>
  ...
  const renderExplorer = () => {
    ...
    const rows = filteredItems.map((item) => ({
      ...
      cells: [
        ...
        `<button class="btn sm" type="button" data-flow-select="${escapeHTML(item.key)}">查看</button>`,
      ],
    }));
    const table = document.getElementById("flowMonitorDetailTable");
    if (table) {
      table.innerHTML = renderTableHTML(
        ["进程", "PID", "协议", "状态", "本地地址", "远端地址", "最后活跃", "入流量", "出流量", "入包", "出包", "操作"],
        rows,
        true,
      );
    }
    setText(
      ...
      `总连接 ${num(all.length)} 条 | 过滤后 ${num(filteredItems.length)} 条 | ${buildFlowMonitorSummary(filteredItems)}`,
    );
    renderDetailPanel();
  };
  ...
  document.getElementById("flowMonitorDetailCopyKeyBtn")?.addEventListener("click", async () => {
    const item = getSelectedItem();
    if (!item) {
      showAppToast("当前无可复制连接", "warning");
      return;
    }
    const ok = await copyText(item.connectionKey || "");
    showAppToast(ok ? "连接键已复制" : "复制连接键失败", ok ? "success" : "error");
  });
  document.getElementById("flowMonitorDetailCopyJsonBtn")?.addEventListener("click", async () => {
    ...
    showAppToast(ok ? "连接 JSON 已复制" : "复制连接 JSON 失败", ok ? "success" : "error");
  });
...
function openFlowMonitorAllModal() {
  ...
  const html = `
    <section class="flow-monitor-modal">
      <div class="row">
        <input id="flowMonitorModalSearch" class="input" placeholder="搜索进程 / PID / 地址 / 协议 / 状态 / 路径" value="${escapeHTML(keyword)}" />
  ...
  `;
  openModal("流量监控全量视图", html, { modalClass: "flow-monitor-modal-full" });
  ...
  const renderInModal = () => {
    ...
    const all = Array.isArray(state.flowMonitorRawItems) ? state.flowMonitorRawItems : [];
    const filtered = applyFlowMonitorFilters(all, {
      keyword: keywordVal,
      protocol: protocolVal,
      status: statusVal,
      sort: sortVal,
    ...
    setText(
      ...
      `总连接 ${num(all.length)} 条 | 过滤后 ${num(filtered.length)} 条 | ${buildFlowMonitorSummary(filtered)}`,
    );
  };
  ...
  document.getElementById("flowMonitorModalTable")?.addEventListener("click", (event) => {
    ...
    const statusVal = String(document.getElementById("flowMonitorModalStatus")?.value || "all").trim().toLowerCase();
    const sortVal = String(document.getElementById("flowMonitorModalSort")?.value || "time_desc").trim().toLowerCase();
    openFlowMonitorExplorerModal(key, "流量连接详情（支持搜索过滤）", {
      keyword: keywordVal,
      protocol: protocolVal,
      status: statusVal,
      sort: sortVal,
